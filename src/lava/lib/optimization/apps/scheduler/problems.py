...
class SatelliteScheduleProblem(SchedulingProblem):
    ...
    def __init__(
        ...
        Assumptions are:
            - The domain of the problem is the unit square with (0, 0) as its
            lower left corner and (1, 1) as the upper right corner.
            - Requests are uniformly randomly distributed over this domain.
            - Satellites are equi-spaced along the Y-axis between y = 0
            and y = 1. This implies that the inter-satellite spacing delta =
        ...
        view_height (float) : viewing window/band of a satellite as a
        multiple of the inter-satellite spacing. For example, a view_height
        of 1.5 implies that a satellite with y-coordinate of y_0 can satisfy
        requests lying within y_0 +/- 1.5 * delta, where delta is the
        inter-satellite spacing. Consequently, view_height <= 1 leads to no
        inter-satellite overlap. Default is 0.25.

        view_coords (numpy.array) : explicitly specified y-coordinates of
        satellites. Default is None, in which case, the y-coordinates are
        calculated assuming equal inter-satellite spacing.

        num_requests (int) : number of requests.

        requests (numpy.array) : explicitly specified (x, y) coordinates of
        requests. Default is None, in which case, the coordinates are
        generated by uniformly sampling in the unit square.

        turn_rate (float) : slew rate/turn rate of a satellite's observation
        equipment, multiplied by the altitude of the satellite. The
        problem formulation Assumes that the altitude of the satellite is
        much greater than any of the length-scales on the ground.

        solution_criteria (float or int) : see `sat_cutoff` in the parent
        class `Scheduler`.
        ...
        super(SatelliteScheduleProblem,
              self).__init__(num_agents=num_satellites,
                             ...
                             seed=seed)
        self.num_satellites: int = self.num_agents
        self.num_requests = self.num_tasks
        self.sat_spacing = 1 / num_satellites

        self.view_height = view_height * self.sat_spacing
        if view_coords is None:
            self.view_coords = np.linspace(0 + self.sat_spacing / 2,
                                           1 - self.sat_spacing / 2,
                                           num_satellites)
        else:
            self.view_coords = view_coords
        self.agent_attrs = list(zip([self.view_height] * num_satellites,
                                    self.view_coords))
        # Sat IDs range from 1 to N_sat, while agent_ids start at 0:
        self.satellites = [agent_id + 1 for agent_id in self.agent_ids]
        self.turn_rate = turn_rate
        self.requests = None
        self.qubo_problem = None
        self.generate_requests(requests)
        self.request_density = self.requests.shape[0]
